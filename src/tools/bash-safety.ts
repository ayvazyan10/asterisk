...
const DANGEROUS_PATTERNS: Array<{ pattern: RegExp; reason: string }> = [
  {
    ...
    reason: 'forced recursive delete on path with /',
  },
  ...
  {
    pattern: /\bchmod\s+(-[a-zA-Z]*R[a-zA-Z]*\s+)?[0-7]*777\s+\/(?!\w)/,
    reason: 'chmod 777 on root',
  },
  {
    pattern: /\bchown\s+(-[a-zA-Z]*R[a-zA-Z]*\s+)?\S+\s+\/(?!\w)/,
    reason: 'chown on root directory',
  },
