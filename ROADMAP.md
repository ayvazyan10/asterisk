...
### ~~Plugin lifecycle hooks~~ ✗ removed
A TypeScript plugin surface shipped alongside the shell hooks, then came out
again. A plan to make its contract as capable as its privileges — a manifest,
a real API, slash commands, a runtime report — was written and cancelled before
any of it was built.

The reason it was removable is the reason it was thin: a plugin ran in-process
with the secret store, the tool registry and the permission gate, and nothing
could confine it, because bubblewrap confines child *processes* and a plugin is
a function call. Everything a plugin could do, an MCP server can do from
outside the process, and Asterisk already speaks MCP as a client. Keeping a
second extension mechanism with none of the isolation and all of the
maintenance was the worse half of that trade.

Shell hooks stay: they are child processes, they hook the same events, and they
have a boundary.
